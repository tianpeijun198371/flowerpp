...
public class MetaDataDOMParser implements ErrorHandler
{
        ...
        public boolean preProcess()
        {
                ...
                if (parser != null)
                {
                        try
                        {
                                ...
                                if (fileName != null)
                                {
                                        ...
                                        {
                                                System.out.println(
                                                        "Non-validated Document parsing complete.");
                                        }
                                }
                        ...
                        catch (SAXException saxexception)
                        {
                                setMessage(MessageType.FAILED,
                                        "MetaDataDOMParser::preProcess()",
                                        "Parser threw a SAXException.");
                                flag = false;
                        }
                ...
                if (flag && (document != null))
                {
                        ...
                        catch (NullPointerException nullpointerexception)
                        {
                                setMessage(MessageType.FAILED,
                                        "MetaDataDOMParser::preProcess()",
                                        "There was a problem parsing the XML with validation off");
                                flag = false;
                        }
                ...
                {
                        setMessage(MessageType.FAILED, "MetaDataDOMParser::preProcess()",
                                "There was a problem parsing the XML with validation off");
                        flag = false;
                }
        ...
        public boolean validate()
        {
                ...
                {
                        setMessage(MessageType.FAILED, "MetaDataDOMParser::validate()",
                                "Parser threw a SAXException - could not reset validation.");
                }
                ...
                {
                        setMessage(MessageType.FAILED, "MetaDataDOMParser::validate()",
                                "IOException caught: could not serialize");
                        inputsource = new InputSource();
                }
                ...
                if ((parser != null) && wellFormed && flag1)
                {
                        ...
                        {
                                setMessage(MessageType.FAILED, "MetaDataDOMParser::validate()",
                                        "SAXException thrown: could not parse");
                        }
                        catch (IOException ioexception1)
                        {
                                setMessage(MessageType.FAILED, "MetaDataDOMParser::validate()",
                                        "IOException thrown: could not parse");
                        }

                        boolean flag2 = errorNotCaught && warningNotCaught &&
                                fatalErrorNotCaught;
        ...
        public void warning(SAXParseException saxparseexception)
        {
                ...
                if (messages.size() < 50)
                {
                        setMessage(MessageType.WARNING, "XMLDocTester::warning()",
                                "Parser warning on line " + saxparseexception.getLineNumber() +
                                        ":  " + saxparseexception.getMessage());
                }
                else if (messages.size() == 50)
                {
                        setMessage(MessageType.INFO, "",
                                "Message limit reached.  Additional parser messages will not be logged.");
                }
        }
        ...
        public void error(SAXParseException saxparseexception)
        {
                ...
                if (messages.size() < 50)
                {
                        setMessage(MessageType.FAILED, "XMLDocTester::error()",
                                "Parser error on line " + saxparseexception.getLineNumber() +
                                        ":  " + saxparseexception.getMessage());
                }
                else if (messages.size() == 50)
                {
        ...
        public void fatalError(SAXParseException saxparseexception)
        {
                ...
                if (messages.size() < 50)
                {
                        setMessage(MessageType.FAILED, "XMLDocTester::fatalError()",
                                "Parser fatal error on line " +
        ...
        protected boolean createParser()
        {
                ...
                catch (SAXException saxexception)
                {
                        parser = null;
                        flag = false;
                        setMessage(MessageType.FAILED, "XMLDocTester::CreateParser()",
                                "Error in setting parser feature.  Parser not created.");
                }
                ...
                switch (word0)
                {
                        ...
                        case 1: // '\001'
                                ...
                                for (int l = 0; l < j; l++)
                                {
                                        ...
                                        if ((s2 != null) && !s2.equals(MDNamespace) &&
                                                !s2.equals(XMLNamespace) && !s2.equals(nsNamespace) &&
                                                !s2.equals(xsiNamespace))
                                        {
                                ...
                                for (int i1 = 0; i1 < nodelist1.getLength(); i1++)
                                {
                                        ...
                                        if ((s4 != null) && !s4.equals(MDNamespace) &&
                                                !s4.equals(XMLNamespace) && !s4.equals(nsNamespace) &&
                                                !s4.equals(xsiNamespace))
                                        {
                                                if (DebugIndicator.ON)
                                                {
        ...
        private String searchFile(String s, String s1)
        {
                ...
                {
                        if ((s.length() > 6) &&
                                (s.substring(0, 5).equals("http:") ||
                                        s.substring(0, 6).equals("https:")))
                        {
                                s2 = s;
                        }
                        else
                        {
                                ...
                                {
                                        setMessage(MessageType.FAILED,
                                                "MetaDataDOMParser::searchFile()",
                                                s1 + " File Not A Normal File.  Verify the" +
                                                        " specified " + s1 + " file, " + s + ", exists.");
                                }
                        }
                }
                catch (NullPointerException nullpointerexception)
                {
                        setMessage(MessageType.FAILED, "MetaDataDOMParser::searchFile()",
                                "Empty " + s1 + " Filename Specified.");
                }
                catch (SecurityException securityexception)
                {
                        setMessage(MessageType.FAILED, "MetaDataDOMParser::searchFile()",
                                s1 + " File Not Accessible.");
                }
