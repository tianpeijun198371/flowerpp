/**
 * Created by IntelliJ IDEA.
 * Delete: dengj
 * Date: Apr 8, 2004
 * Time: 9:12:45 AM
 * To change this template use Options | File Templates.
 */
package com.ulearning.ulms.tools.delete.dao;

import com.ulearning.ulms.tools.delete.exceptions.DeleteDAOSysException;
import com.ulearning.ulms.util.DBUtil;


public class DeleteDAOFactory
{
        /**
         * This method instantiates a particular subclass implementing
         * the DAO methods based on the information obtained from the
         * deployment descriptor
         */
        public static DeleteDAO getDAO() throws DeleteDAOSysException
        {
                DeleteDAO dao = null;

                try
                {
                        dao = new DeleteDAOImpl();

                        /*
                          if (DBUtil.getWhichDatabase() == 0)
                          {
                                  dao = new DeleteDAOOracle();
                          }
                          else if (DBUtil.getWhichDatabase() == 1) //"Microsoft SQL Server"))
                          {
                                  //dao = new DeleteDAOSQLServer();
                          }
                        */
                }
                catch (Exception se)
                {
                        throw new DeleteDAOSysException(
                                "DeleteDAOFactory.getDAO:  Exception while getting DAO type : \n" +
                                        se.getMessage());
                }

                return dao;
        }
}
